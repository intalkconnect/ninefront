...
// Mescla SEM nunca sobrescrever com undefined/null
function mergeConvSafe(mergeConversation, user_id, patch) {
  const safe = {};
  for (const [k, v] of Object.entries(patch || {})) {
    if (v !== undefined && v !== null) safe[k] = v;
  }
  mergeConversation(user_id, safe);
}
...
export default function Atendimento() {
  ...
  const handleNewMessage = useCallback(
    async (message) => {
      ...
      // ✅ Atualiza SEMPRE o card (sem sobrescrever campos críticos com undefined)
      mergeConvSafe(mergeConversation, message.user_id, {
        ticket_number: message.ticket_number || message.ticket,
        timestamp: message.timestamp,
        content: contentToText(message.content),
        channel: message.channel || "whatsapp",
        direction: message.direction,
        assigned_to: message.assigned_to, // só entra se vier definido
        status: message.status,           // idem
        fila: message.fila,               // idem
      });
