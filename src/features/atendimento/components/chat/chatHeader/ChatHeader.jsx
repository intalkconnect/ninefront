...
async function saveCustomerTagsDiff(userId, initialTags = [], selectedTags = []) {
  ...
  if (toAdd.length) await apiPost(`/tags/customer/${encodeURIComponent(userId)}`, { tags: toAdd });
  for (const t of toRemove) await apiDelete(`/tags/customer/${encodeURIComponent(userId)}/${encodeURIComponent(t)}`);
}
...
export default function ChatHeader({ userIdSelecionado, clienteInfo }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { if (alive) setTicketCatalog([]); }
    })();
    return () => { alive = false; };
  }, [ticketNumber, flowId]);
  ...
  const finalizarAtendimento = async () => {
    const ok = await confirm({
      ...
      confirmText: 'Finalizar', cancelText: 'Cancelar', tone: 'success',
    });
    if (!ok) return;
    try {
      ...
      const closeUrl = withFlow(`/tickets/${encodeURIComponent(userId)}`, flowId);
      ...
      await confirm({ title: 'Erro ao finalizar', description: 'Não foi possível concluir a operação. Tente novamente.', confirmText: 'Ok', hideCancel: true, tone: 'danger' });
    }
  };
  ...
  return (
    <>
      ...
      {ticketCatalog.length > 0 && (
        <div className="tag-subbar">
          <div className="tag-subbar__inner">
            <ComboTags options={ticketCatalog} selected={ticketTags} onAdd={addTicketTag} placeholder="Procurar tag..." />
            {ticketTags.length > 0 && (
              <div className="chips">
                {ticketTags.map(t => (
      ...
      {showTransferModal && (
        <TransferModal userId={userId} flowId={flowId} onClose={() => setShowTransferModal(false)} />
      )}
    </>
  );
}
