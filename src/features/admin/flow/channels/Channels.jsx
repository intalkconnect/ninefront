...
export default function Channels() {
    ...
    (async () => {
      try {
        ...
        if (fs && fs.ok) {
          ...
          setFb((prev) => ({ ...prev, loading: false, connected: false }));
      } catch {
        setFb({
          loading: false,
          connected: false,
    ...
    (async () => {
      try {
        ...
        if (is && is.ok) {
          ...
          setIg((prev) => ({ ...prev, loading: false, connected: false }));
      } catch {
        setIg({
          loading: false,
          connected: false,
  ...
  useEffect(() => {
    ...
    const expectedOrigin = (() => {
      try {
        return new URL(AUTH_ORIGIN).origin;
      } catch {
        return AUTH_ORIGIN;
      }
    })();
          ...
          .then((fin) => {
            if (fin?.ok && (fin?.connected || fin?.page_id)) {
              setFb((s) => ({
                ...
                connected: true,
                loading: false,
                pageId: fin.page_id || s.pageId,
                pageName: fin.page_name || s.pageName,
              }));
        ...
        apiPost("/instagram/finalize", { subdomain: sub, code, redirect_uri })
          .then(async (res) => {
            if (res?.ok && res?.step === "pages_list") {
              ...
              return apiPost("/instagram/finalize", {
                subdomain: sub,
                redirect_uri,
                page_id: pick.id,
                user_token: res.user_token,
          ...
          .then((fin) => {
            if (fin?.ok && (fin?.connected || fin?.ig_user_id)) {
              setIg((s) => ({
                ...
                pageId: fin.page_id || s.pageId,
                pageName: fin.page_name || s.pageName,
                igUserId: fin.ig_user_id || s.igUserId,
                igUsername: fin.ig_username || s.igUsername,
              }));
  ...
  const handleWaOAuthCode = useCallback(
    async ({ code, stateB64, redirectUri }) => {
      ...
      try {
        ...
        const res = await apiPost("/whatsapp/embedded/es/finalize", {
          subdomain: sub,
          code,
          redirect_uri: redirectUri, // ✅ repassa redirect_uri pro backend
        });
            ...
            {!wa.connected ? (
              <>
                <p className={styles.cardDesc}>
                  Conecte via <strong>Meta Embedded Signup</strong> (popup
                  único) e finalize a configuração.
                </p>
