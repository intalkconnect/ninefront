...
const IconWA = (props) => (
  <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" {...props}>
    <path
      fill="#25D366"
      d="M20.52 3.48A11.94 11.94 0 0 0 12 0C5.37 0 0 5.37 0 12c0 2.11.55 4.16 1.6 5.98L0 24l6.2-1.62A11.93 11.93 0 0 0 12 24c6.63 0 12-5.37 12-12 0-3.2-1.25-6.22-3.48-8.52z"
    />
    <path
      fill="#fff"
...
export default function Customers() {
              ...
              slice.map(async (row) => {
                ...
                try {
                  ...
                  const r = await apiGet(
                    `/tags/customer/${encodeURIComponent(
                      uid
                    )}?${qsTags.toString()}`
                  );
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      const list = raw
        ...
        .filter((x) => x.tag);
      const dedup = Array.from(new Map(list.map((x) => [x.tag, x])).values()).sort(
  ...
  return (
    <div className={styles.page}>
      {/* HEADER NO PADRÃO DO FLOW (igual Agents) */}
